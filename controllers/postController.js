...
const editPost = async(req, res , next) =>{
    try {
        ...
        const postId = req.params.id;
        if(!postId){
            return next(new HttpError("Post unavailabel.", 400))
        }
        ...
        if(req.user.id == oldPost.creator){
            if(!req.files){
                ...
                const {thumbnail} = req.files;
                    ...
                    updatedPost = await Post.findByIdAndUpdate(postId, {title, category, description, thumbnail : newFilename},{name: true});
            }
        }
